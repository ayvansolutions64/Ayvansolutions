...
import { itemReveal, sectionReveal, softViewport } from '../lib/motion';
...
function VisionItem({ item }: { item: typeof visionItems[0] }) {
  const ref = useRef<HTMLDivElement>(null);
  const { scrollYProgress } = useScroll({ target: ref, offset: ['start 0.9', 'start 0.5'] });
  const x = useTransform(scrollYProgress, [0, 1], [22, 0]);
  ...
  return (
    <motion.div
      ref={ref}
      style={{ x, opacity: op, scale, filter }}
...
export default function Vision() {
  ...
  const sectionOpacity = useTransform(scrollYProgress, [0, 0.16, 0.84, 1], [0, 1, 1, 0.18]);
  const sectionY = useTransform(scrollYProgress, [0, 0.22, 0.82, 1], [54, 0, 0, -36]);
  ...
  return (
    <section id="vision" ref={ref} className="relative min-h-screen flex items-center py-32">
      ...
      <motion.div style={{ opacity: sectionOpacity, y: sectionY }} className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 w-full">
        <motion.div style={{ opacity: labelOpacity }} className="mb-16">
